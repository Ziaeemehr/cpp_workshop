...
void acquire_signal(double *signal, double *theta, int fs) {
     ...
     * amplitudes.
     */
    std::default_random_engine gen_uniform(1234);
    std::uniform_real_distribution<double> rand_uniform(0.0,1.0);

    int i;
    for (i = 0; i < NUM_POINTS; ++i) {
        theta[i] = (double)i / (double)fs;
        signal[i] = 1.0*sin(1.5 * 2.0 * M_PI * theta[i]) +\
        0.01*rand_uniform(gen_uniform);
    }
}

int main() {
    int fs = 100;  // Sample rate in Hz
    unsigned flags{0};
